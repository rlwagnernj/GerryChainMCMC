...
def RunGerryChain(Graph, Chain, VTD_Data='', Checkpoint = None, Print_Iterations = False, Output_Csv = '', Output_Metadata = '', Output_Seg=''):
    ...
    if Checkpoint is not None:
        ...
        GerryChainOutput(Chain_List=chain_list_all, Metadata_List=metadata_list_all, Output_Csv = Output_Csv + 'all', Output_Metadata=Output_Metadata + 'all')
        with open(Output_Seg + 'all', 'x', encoding='utf8', newline='') as output_file:
            seg_df.to_csv(output_file, sep=',', encoding='utf-8', header=True)
...
#def ReUseChainOutput(GDF, Transposed_Csv):

    #n = Transposed_Csv.shape[1]
    #districts = Transposed_Csv.rename(columns=Transposed_Csv.iloc[0]).drop(Transposed_Csv.index[0])
    
    #merged_gdf = GDF.merge(right = districts[n], left_on = 'vtd_cd_117', right_index = True).drop(columns='cd_117').rename(columns={10:'cd_117'})
